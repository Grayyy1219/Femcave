...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $deductQuantityQuery = "UPDATE items SET Quantity = Quantity - $Quantity WHERE ItemID = $productId";
        $resultDeductQuantity = mysqli_query($con, $deductQuantityQuery);
        $saveOrderQuery = "UPDATE items SET Solds = Solds + $Quantity WHERE ItemID = $productId";
    ...
    echo "$selectedItems";
    header('Location: ordersuccessful2.php');
    exit();
}
?>
<!DOCTYPE html>
