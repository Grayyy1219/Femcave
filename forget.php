...
if ($result->num_rows > 0) { // Fix: use > 0 instead of > 1
    try {
        ...
        $mail->setFrom("$resetemail", 'Femcave');
        $mail->addAddress("$resetemail", '');

        $reset_link = "http://localhost/Femcave/resetpassword.php?email=$resetemail";
        $message = '
        <html>
        <body>
            <p>Dear Maam/Sir,</p>
    ...
    ';
        ...
        $mail->isHTML(true);
        $mail->Subject = 'Important: Reset Your Femcave Account Password Now!';
        $mail->Body    = $message;
